...
const CartPage = ({ mode = 'dark' }) => {
  ...
  const handleRemove = (name) => {
    removeFromCart(name);
    setSnackbarMsg('Removed from cart!');
  ...
  return (
    ...
    >
      <Paper elevation={8} sx={{
        ...
        {cart.length === 0 ? (
          <Typography sx={{ color: getTextColor(mode), textAlign: 'center', mt: 4 }}>No items in cart.</Typography>
        ) : (
        ...
        {cart.length > 0 && (
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
            <Button
              ...
              color="primary"
              size="large"
              sx={{ borderRadius: 2, fontWeight: 600, px: 5 }}
              onClick={() => window.location.href = '/payment-method'}
            >
              Checkout
            </Button>
          </Box>
